...
class LatentDiffusion_audio(DDPM):
    ...
    def on_train_batch_start(self, batch, batch_idx):
        ...
        if self.scale_by_std and self.current_epoch == 0 and self.global_step == 0 and batch_idx == 0 and not self.restarted_from_ckpt:
            ...
            z = self.get_first_stage_encoding(
                encoder_posterior).detach()  # get latent
        ...
        if self.model.conditioning_key is not None:
            ...
            if cond_key == 'masked_image':
                ...
                cc = torch.nn.functional.interpolate(
                    mask, size=c.shape[-2:])  # [B, 1, 10, 106]
    ...
    def test_step(self, batch, batch_idx):
        ...
        enc_emb = self.sample(cond, batch_size, timesteps=self.num_timesteps)
    ...
    def p_losses(self, x_start, cond, t, noise=None):
        ...
        loss_simple = self.get_loss(
            model_output, target, mean=False).mean(dim=mean_dims)
    ...
    def sample_log(self, cond, batch_size, ddim, ddim_steps, **kwargs):
        ...
        if ddim:
            ...
            shape = (self.channels, self.mel_dim, self.mel_length) if self.channels > 0 else (
                self.mel_dim, self.mel_length)
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        log["reconstruction"] = xrec if len(
            xrec.shape) == 4 else xrec.unsqueeze(1)
        if self.model.conditioning_key is not None:
            if hasattr(self.cond_stage_model, "decode") and self.cond_stage_key != "masked_image":
                ...
                xc = self.cond_stage_model.decode(
                    c[:, :self.cond_stage_model.embed_dim, :, :])
                log["conditioning"] = xc
        ...
        if sample:
            ...
            log["samples"] = x_samples if len(
                x_samples.shape) == 4 else x_samples.unsqueeze(1)
            ...
            if quantize_denoised and not isinstance(self.first_stage_model, AutoencoderKL) and not isinstance(
                ...
                log["samples_x0_quantized"] = x_samples if len(
                    x_samples.shape) == 4 else x_samples.unsqueeze(1)
