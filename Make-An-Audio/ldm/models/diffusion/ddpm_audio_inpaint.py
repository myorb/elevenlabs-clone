...
class LatentDiffusion_audioinpaint(DDPM):
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
        ...
        if self.model.conditioning_key is not None:  # 'crossattn' for txt2image, 'hybird' for txt_inpaint
            ...
            if self.model.conditioning_key == 'hybrid':
                ...
                masked_image, mask = masked_image.to(
                    self.device), mask.to(self.device)
                masked_image = self.get_first_stage_encoding(
                    self.encode_first_stage(masked_image)).detach()
                resized_mask = torch.nn.functional.interpolate(
                    mask, size=masked_image.shape[-2:])
                ...
                if cond_key != self.first_stage_key:
                    ...
                    xc = x
            # cond_stage_trainable is true for txt2img，force_c_encoder = True，when called in log_images
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, list):
                    # import pudb; pudb.set_trace()
                    # 因为log_images内接下来要调用sample_log,所以需要预先得到处理好的c
                    c = self.get_learned_conditioning(xc)
                if isinstance(xc, dict):
                    c = {}
                    c['c_concat'] = xc['c_concat']
                    c['c_crossattn'] = self.get_learned_conditioning(
                        xc[cond_key])
                else:
    ...
    def test_step(self, batch, batch_idx):
        ...
        cond[self.cond_stage_key] = self.get_learned_conditioning(
            cond[self.cond_stage_key])  # c: string -> [B, T, Context_dim]
        ...
        savedir = os.path.join(self.trainer.log_dir,
                               f'output_imgs_{test_ckpt_path}', 'fake_class')
        ...
        for k in range(reconstructions.shape[0]):
            ...
            vname_num_split_index = file_names[b].rfind(
                '_')  # file_names[b]:video_name+'_'+num
            v_n, num = file_names[b][:vname_num_split_index], file_names[b][vname_num_split_index+1:]
            # the num_th caption, the repeat_th repitition
            save_img_path = os.path.join(
                savedir, f'{v_n}_sample_{num}_{repeat}.npy')
    ...
    def forward(self, x, c, *args, **kwargs):
        ...
        if self.model.conditioning_key is not None:
            ...
            if self.cond_stage_trainable:
                if isinstance(c, dict):
                    c[self.cond_stage_key] = self.get_learned_conditioning(
                        c[self.cond_stage_key])
                    c['c_crossattn'] = c.pop(self.cond_stage_key)
                else:
                    # c: string -> [B, T, Context_dim]
    ...
    def apply_model(self, x_noisy, t, cond, return_ids=False):
        ...
        if hasattr(self, "split_input_params"):
            ...
            # tensor with shape [b,c,mel_len,T]
    ...
    def log_images(self, batch, N=8, n_row=4, sample=True, ddim_steps=200, ddim_eta=1., return_keys=None,
        ...
        if self.model.conditioning_key is not None:
            # when cond_stage is first_stage. (bert embedder doesnot have decode)
            if hasattr(self.cond_stage_model, "decode"):
                ...
                if self.model.conditioning_key == 'hybrid':
                    # c_concat is the concat result of masked_img latent and resized mask. get latent here to decode
                    log["decoded_maskedimg"] = self.first_stage_model.decode(
                        c['c_concat'][:, :self.first_stage_model.embed_dim])
